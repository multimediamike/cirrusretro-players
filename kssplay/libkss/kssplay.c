/*
 * kssplay.c written by Mitsutaka Okazaki(okazaki@angel.ne.jp)
 */
...
inline k_int32 apply_volume(k_int32 data, k_int32 vol)
{
...
{
  if(kssplay->fade_flag&&!kssplay->fade) return KSSPLAY_FADE_END ;
  else return kssplay->fade_flag ;
}
...
{
  kssplay->fade_flag = KSSPLAY_FADE_NONE ;
  kssplay->fade = 0 ;
}

inline k_int32 fader(KSSPLAY *kssplay, k_int32 sample)
{
  if(kssplay->fade_flag)
  {
    if(kssplay->fade > kssplay->fade_step)
    {
      kssplay->fade -= kssplay->fade_step ;
      return (k_int16)((sample*(kssplay->fade>>FADE_BASE_BIT)) >> FADE_BIT) ;
    }
    else
    {
      kssplay->fade = 0 ;
...
inline int clip(int min, int val, int max)
{
  ...
  for(i=0;i<length;i++)
  {
    ...
    if(!kssplay->device_mute[EDSC_PSG])
    {
      if(kssplay->kss->sn76489)
        ...
        c = FIR_calc(kssplay->device_fir[0][EDSC_PSG],PSG_calc(kssplay->vm->psg));
    ...
    if(!kssplay->device_mute[EDSC_SCC])
    {
      c = FIR_calc(kssplay->device_fir[0][EDSC_SCC],SCC_calc(kssplay->vm->scc));
...
{
  VM_exec(kssplay->vm, kssplay->vm->clock / kssplay->rate  * length ) ;
...
static void get_psg_status(KSSPLAY *kssplay, KSSPLAY_CH_STATUS sch[3])
{
  ...
  if(psg)
  {
    for(i=0;i<3;i++)
    {
      sch[i].fnum = (psg->reg[(i<<1)+1]<<8) | psg->reg[i*2] ;
      sch[i].freq = (double)psg->clk/sch[i].fnum ;
      if(psg->reg[8+i]&16)
      {
        sch[i].volume = 15;
        sch[i].voice = psg->reg[8+i]&15;
      }
      else
      {
        sch[i].voice = 0;
        sch[i].volume = psg->reg[8+i];
      }
    }
  }
}

static void get_scc_status(KSSPLAY *kssplay, KSSPLAY_CH_STATUS sch[5])
{
}

static void get_opll_status(KSSPLAY *kssplay, KSSPLAY_CH_STATUS sch[9])
{
}

static void get_opl_status(KSSPLAY *kssplay, KSSPLAY_CH_STATUS sch[9])
{
}

void KSSPLAY_get_status(KSSPLAY *kssplay, KSSPLAY_STATUS *kstat)
{
  get_psg_status(kssplay, &(kstat->ch[0]));
  get_scc_status(kssplay, &(kstat->ch[3]));
  get_opll_status(kssplay, &(kstat->ch[12]));
  get_opl_status(kssplay, &(kstat->ch[21]));
}
